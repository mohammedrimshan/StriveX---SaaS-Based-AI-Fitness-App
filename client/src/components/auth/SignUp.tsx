...
const currentValidationSchema = getValidationSchema(userType, currentStep);
	...
	const handleAddOtherSkill = () => {
		if (otherSkill.trim() && !formik.values.skills.includes(otherSkill.trim())) {
			formik.setFieldValue("skills", [...formik.values.skills, otherSkill.trim()]);
			setOtherSkill("");
		}
	};
		...
		<div className="flex flex-col gap-3.5">
			...
			<LocalizationProvider dateAdapter={AdapterDateFns}>
				<DatePicker
					...
					format="dd-MM-yyyy" // Set date format to date-month-year
					slotProps={{
						textField: {
							id: "dateOfBirth",
							name: "dateOfBirth",
			...
			>
				<InputLabel id="gender-label" sx={{ "&.Mui-focused": { color: "var(--violet)" } }}>Gender</InputLabel>
				<Select
					...
					<motion.div
						...
						<div className="text-center mb-8">
							<h2 className="text-3xl font-bold tracking-tight">
								Create your{" "}
								{userType === "client" ? "" : userType === "trainer" ? "trainer" : ""} account
						...
						<form
							...
							<AnimatePresence mode="wait">
								{userType === "trainer" && currentStep === 2 ? (
									...
									<motion.div
										key="basic-info"
										initial={{ x: userType === "trainer" ? -300 : 0, opacity: userType === "trainer" ? 0 : 1 }}
										animate={{ x: 0, opacity: 1 }}
										exit={{ x: 300, opacity: 0 }}
										transition={{ type: "spring", stiffness: 300, damping: 30 }}
									>
							...
							<div className={`flex ${userType === "trainer" && (currentStep === 2 || currentStep === 3) ? "justify-between" : "justify-center"} gap-4`}>
								{userType === "trainer" && (currentStep === 2 || currentStep === 3) && (
									<Button
										type="button"
								...
								<Button
									...
									fullWidth={!(userType === "trainer" && (currentStep === 2 || currentStep === 3))}
									variant="contained"
									endIcon={(userType === "trainer" && (currentStep === 1 || currentStep === 2)) ? <ArrowRight size={16} /> : undefined}
									sx={{
