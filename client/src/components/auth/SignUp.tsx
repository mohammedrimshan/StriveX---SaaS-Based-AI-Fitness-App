...
import { IUser as UserType } from "@/types/User";
...
const SignUp = ({
  ...
  const [userData, _] = useState<UserType>({} as UserType);
  const [currentStep, setCurrentStep] = useState(1);
  const [otherSkill, setOtherSkill] = useState("");
const [__, setFormData] = useState<UserFormData>(/* initial values */);
      ...
      <LocalizationProvider dateAdapter={AdapterDateFns}>
        <DatePicker
          ...
          onChange={(newValue: Date | null) => {
            if (newValue) {
              formik.setFieldValue("dateOfBirth", newValue.toISOString());
            } else {
