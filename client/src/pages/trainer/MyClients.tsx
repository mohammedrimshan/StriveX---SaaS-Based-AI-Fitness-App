...
import { useTrainerClients } from '@/hooks/trainer/useGetAllTrainers';
...
const MyClients: React.FC = () => {
  ...
  const { data, isLoading, isError, error } = useTrainerClients(page, limit);
  ...
  return (
    <div className="min-h-screen py-10 bg-gradient-to-br from-white to-purple-50 dark:from-gray-900 dark:to-gray-800">
      <div className="container mx-auto px-4">
        ...
        >
          <motion.div 
            className="inline-block mb-4 p-2 rounded-full bg-purple-100 dark:bg-purple-900/30"
            initial={{ scale: 0.8 }}
            animate={{ scale: 1 }}
          ...
          <motion.h1
            className="text-3xl sm:text-4xl font-bold mb-2 bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.2, duration: 0.5 }}
          >
            Fitness Client Directory
          </motion.h1>
          <motion.p
          ...
          >
            Browse and manage your fitness clients. Click on a client to view detailed information.
          </motion.p>
        </motion.div>
        ...
        >
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="pl-10 p-2 rounded-lg border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200"
              />
            </div>
            <Select onValueChange={setFitnessGoalFilter} value={fitnessGoalFilter}>
              <SelectTrigger className="w-full sm:w-48 border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200">
                <SelectValue placeholder="Filter by Goal" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All Goals</SelectItem>
                <SelectItem value="weightLoss" className="flex items-center gap-2">
                  <div className="flex items-center gap-2">
                    <Scale className="w-4 h-4 text-rose-500" />
                    <span>Weight Loss</span>
                  </div>
                </SelectItem>
                <SelectItem value="muscleGain" className="flex items-center gap-2">
                  <div className="flex items-center gap-2">
          ...
          {fitnessGoalFilter && (
            ...
            >
              <Badge 
                className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-100 text-purple-800 dark:bg-purple-900 dark:text-purple-100 hover:bg-purple-200 dark:hover:bg-purple-800 transition-colors"
        ...
        {!isLoading && !isError && filteredClients.length > 0 && (
          ...
          >
            <motion.div
              className="text-center mb-6 bg-gradient-to-r from-purple-600 to-pink-600 text-white py-3 px-6 rounded-xl shadow-md"
              initial={{ scale: 0.95 }}
              animate={{ scale: 1 }}
              transition={{ delay: 0.2, duration: 0.5 }}
            >
              <span className="font-semibold">Training {filteredClients.length} Client{filteredClients.length !== 1 ? 's' : ''}!</span> Keep up the great work!
            </motion.div>
