...
export function UserBookings({ bookings, onBookingCancelled }: UserBookingsProps) {
  ...
  const processedBookings = useMemo(() => {
    return bookings.map(booking => {
      ...
      const joinAvailable = isAfter(sessionDate, currentTime) && isBefore(sessionDate, addMinutes(currentTime, 10))
  ...
  const handleJoinSession = (slotId: string) => {
    try {
      ...
      errorToast("Failed to open session. Please check your popup blocker settings.")
    }
  }
  ...
  const upcomingSessions = processedBookings.filter(booking => !booking.isPast)
    ...
    <AnimatePresence>
      <div className="space-y-6">
        ...
        {upcomingSessions.length > 0 && (
          <div>
            <h2 className="text-lg font-medium mb-3 text-gray-800">Upcoming Sessions</h2>
        ...
        {pastSessions.length > 0 && (
          <div className="mt-6">
            <h2 className="text-lg font-medium mb-3 text-gray-700">Past Sessions</h2>
...
function SessionCard({ booking, isCancelling, onCancel, onJoin, isPast = false }: SessionCardProps) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, x: -20 }}
      className={`p-4 rounded-xl border-2 ${isPast 
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
        <div className="mb-3 sm:mb-0">
          ...
          <div className="mt-2 text-sm text-gray-700 flex items-center">
            ...
            <span className="font-medium">Trainer:</span> {booking.trainerName || "Your Trainer"}
          </div>
        </div>
        ...
        {!isPast ? (
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 w-full sm:w-auto">
            <Button
              ...
              disabled={!booking.joinAvailable}
