...
export const exerciseSchema = Yup.object().shape({
  name: Yup.string()
    ...
    .matches(
      /^[a-zA-Z0-9\s.,'-]+$/,
      "Exercise name cannot contain special characters"
    )
    .test(
      "not-only-whitespace",
      "Exercise name cannot be empty or just spaces",
  ...
  videoFile: Yup.mixed<File>()
    ...
    .test(
      ...
      (value) => !value || value.size <= 100 * 1024 * 1024 // 100MB in bytes
    )
    .test(
      "file-type",
      ...
      (value) =>
        !value ||
        ["video/mp4", "video/quicktime", "video/webm"].includes(value.type)
    ),
});