...
@injectable()
export class BackupTrainerInvitationRepository extends BaseRepository<IBackupTrainerInvitationEntity> implements IBackupTrainerInvitationRepository {
  constructor() {
    super(BackupTrainerInvitationModel);
  }

  async findByClientId(clientId: string): Promise<IBackupTrainerInvitationEntity[]> {
    const invitations = await this.model.find({ clientId }).lean();
    return invitations.map((inv) => this.mapToEntity(inv));
  }

  async findPendingByClientId(clientId: string): Promise<IBackupTrainerInvitationEntity[]> {
    const invitations = await this.model.find({
      clientId,
  ...
  async updateStatus(id: string, status: BackupInvitationStatus, respondedAt?: Date): Promise<IBackupTrainerInvitationEntity | null> {
